...
static naRef f_main_iteration_do(naContext ctx, naRef me, int argc, naRef* args)
{
    naRef blocking = (argc && naIsNum(args[0]) ) ? naNumValue(args[0]) : naNum(0);
    naModUnlock();
     while ( gtk_events_pending () )
	gtk_main_iteration_do( blocking.num );
    naModLock();
    return naNil();
}
...
//core stuff
    ...
    { "init", f_init },
    { "main", f_main },
    { "main_iteration_do", f_main_iteration_do },
